...
| ID | Date | Reporter | Summary | Area | Repro / notes | Status |
|---|---|---|---|---|---|---|
| U2 | 2026-08-09 | Leah | Nowhere obvious for testers to switch roles / see other role views | Owner Admin / Multi-role | Admin View As is owner-only. Testers need Multi-role Switch View (header Switch View). Default multiRole on for new testers. go16: invite accept returns + applies `multiRoleTester` so Switch View shows immediately after signup (local E2E verified) | fixed-on-testing |
