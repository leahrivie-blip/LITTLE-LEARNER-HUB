...
} = require("../scripts/curriculum-preschool-summer-import-targets.js");
...
function plansNeedingRepair(curriculum, targets = PRESCHOOL_SUMMER_IMPORT_TARGETS) {
  ...
  for (const target of targets) {
    ...
    let source;
    try {
      source = readPreschoolSummerImportTarget(target);
    } catch {
      continue;
    }
...
async function ensurePreschoolSummerCurriculumSeeded(deps) {
  const {
    readStore,
    writeStoreAsync,
    writeSiteCurriculum,
  ...
  const missing = preschoolSummerPlansMissing(curriculum);
  ...
  if (!missing.length && !repairQueue.length) {
    return {
      seeded: 0,
      repaired: 0,
      skipped: PRESCHOOL_SUMMER_IMPORT_TARGETS.length,
      errors: [],
    };
  }
  ...
  for (const entry of repairQueue) {
    try {
      ...
      if (applyPlan(entry.target, entry.source, existingPlan)) {
        repaired += 1;
        console.log(
          `[curriculum-preschool-summer-seed] repaired ${entry.target.stableId} (${entry.reason}; ${entry.liveTotal} → ${entry.sourceTotal} activities)`,
        );
      }
    } catch (error) {
  ...
  if (seeded > 0 || repaired > 0) {
    ...
    console.log(
      `[curriculum-preschool-summer-seed] seeded ${seeded} · repaired ${repaired} preschool summer lesson plan(s)`,
    );
  }

  if (errors.length) {
    console.error("[curriculum-preschool-summer-seed] errors:", errors);
  }

  return {
    seeded,
    repaired,
    skipped: PRESCHOOL_SUMMER_IMPORT_TARGETS.length - missing.length - repaired,
    errors,
  };
}

module.exports = {
  ensurePreschoolSummerCurriculumSeeded,
  plansNeedingRepair,
  PRESCHOOL_SUMMER_IMPORT_TARGETS,
};
