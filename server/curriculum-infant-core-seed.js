/**
 * Startup seed/repair for infant jul2026 core plans that shipped with Mon/Tue only.
 */
const {
  INFANT_CORE_IMPORT_TARGETS,
  readInfantCoreImportTarget,
  infantCorePlansMissing,
} = require("../scripts/curriculum-infant-core-import-targets.js");
...
function plansNeedingRepair(curriculum, targets = INFANT_CORE_IMPORT_TARGETS) {
  ...
  for (const target of targets) {
    ...
    let source;
    try {
      source = readInfantCoreImportTarget(target);
    } catch {
      continue;
    }
...
async function ensureInfantCoreCurriculumSeeded(deps) {
  const {
    readStore,
    writeStoreAsync,
    writeSiteCurriculum,
  ...
  const missing = infantCorePlansMissing(curriculum);
  ...
  if (!missing.length && !repairQueue.length) {
    return {
      seeded: 0,
      repaired: 0,
      skipped: INFANT_CORE_IMPORT_TARGETS.length,
      errors: [],
    };
  }
  ...
  for (const target of missing) {
    try {
      const parsed = readInfantCoreImportTarget(target);
  ...
  for (const entry of repairQueue) {
    try {
      ...
      if (applyPlan(entry.target, entry.source, existingPlan)) {
        repaired += 1;
        console.log(
          `[curriculum-infant-core-seed] repaired ${entry.target.stableId} (${entry.reason}; ${entry.liveTotal} → ${entry.sourceTotal} activities)`,
        );
      }
    } catch (error) {
  ...
  if (seeded > 0 || repaired > 0) {
    ...
    console.log(
      `[curriculum-infant-core-seed] seeded ${seeded} · repaired ${repaired} infant core lesson plan(s)`,
    );
  }

  if (errors.length) {
    console.error("[curriculum-infant-core-seed] errors:", errors);
  }

  return {
    seeded,
    repaired,
    skipped: INFANT_CORE_IMPORT_TARGETS.length - missing.length - repaired,
    errors,
  };
}

module.exports = {
  ensureInfantCoreCurriculumSeeded,
  plansNeedingRepair,
  infantCorePlansMissing,
};
