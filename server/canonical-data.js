...
function describeCanonicalHomes() {
  return {
    ...
    FormsAssigned: "child blob Documents (child/family) + programData[programId].forms.staffDocuments (authoritative; client programSettings.staffFormDocuments is temporary read-only fallback)",
    FormsLibrary: "system: formGroups/siteContent.forms; provider: programData[programId].forms.templates (authoritative; client programSettings.formTemplates temporary read-only fallback)",
    FormsSignatures: "same Document/staffDocuments row (signedAt/By/Role, bodyHash, contentVersion, signedSnapshot)",
    FormsAudit: "store.formsAudit append-only (+ formsAuditArchive for preservation moves; never destructive FIFO)",
