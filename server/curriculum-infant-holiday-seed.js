/**
 * Startup seed/repair: ensure Infant holiday Pro published lesson plans exist
...
} = require("../scripts/curriculum-infant-holiday-import-targets.js");
...
function plansNeedingRepair(curriculum, targets = INFANT_HOLIDAY_IMPORT_TARGETS) {
  ...
  for (const target of targets) {
    ...
    let source;
    try {
      source = readInfantHolidayImportTarget(target);
    } catch {
      continue;
    }
...
async function ensureInfantHolidayCurriculumSeeded(deps) {
  const {
    readStore,
    writeStoreAsync,
    writeSiteCurriculum,
  ...
  const missing = infantHolidayPlansMissing(curriculum);
  ...
  if (!missing.length && !repairQueue.length) {
    return {
      seeded: 0,
      repaired: 0,
      skipped: INFANT_HOLIDAY_IMPORT_TARGETS.length,
      errors: [],
    };
  }
  ...
  for (const entry of repairQueue) {
    try {
      ...
      if (applyPlan(entry.target, entry.source, existingPlan)) {
        repaired += 1;
        console.log(
          `[curriculum-infant-holiday-seed] repaired ${entry.target.stableId} (${entry.reason}; ${entry.liveTotal} → ${entry.sourceTotal} activities)`,
        );
      }
    } catch (error) {
  ...
  if (seeded > 0 || repaired > 0) {
    ...
    console.log(
      `[curriculum-infant-holiday-seed] seeded ${seeded} · repaired ${repaired} infant holiday lesson plan(s)`,
    );
  }

  if (errors.length) {
    console.error("[curriculum-infant-holiday-seed] errors:", errors);
  }

  return {
    seeded,
    repaired,
    skipped: INFANT_HOLIDAY_IMPORT_TARGETS.length - missing.length - repaired,
    errors,
  };
}

module.exports = {
  ensureInfantHolidayCurriculumSeeded,
  plansNeedingRepair,
  INFANT_HOLIDAY_IMPORT_TARGETS,
};
