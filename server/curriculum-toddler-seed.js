...
/**
 * Startup seed/repair: ensure Toddler Pro published lesson plans exist
 * and repair incomplete weeks.
 *
 * Idempotent:
...
const {
  ...
  toddlerPlansMissing,
} = require("../scripts/curriculum-toddler-import-targets.js");
...
function plansNeedingRepair(curriculum, targets = TODDLER_IMPORT_TARGETS) {
  ...
  for (const target of targets) {
    ...
    let source;
    try {
      source = readToddlerImportTarget(target);
    } catch {
      continue;
    }
...
async function ensureToddlerCurriculumSeeded(deps) {
  const {
    readStore,
    writeStoreAsync,
    writeSiteCurriculum,
  ...
  const missing = toddlerPlansMissing(curriculum);
  ...
  if (!missing.length && !repairQueue.length) {
    return {
      seeded: 0,
      repaired: 0,
      skipped: TODDLER_IMPORT_TARGETS.length,
      errors: [],
    };
  }
  ...
  for (const entry of repairQueue) {
    try {
      ...
      if (applyPlan(entry.target, entry.source, existingPlan)) {
        repaired += 1;
        console.log(
          `[curriculum-toddler-seed] repaired ${entry.target.stableId} (${entry.reason}; ${entry.liveTotal} → ${entry.sourceTotal} activities)`,
        );
      }
    } catch (error) {
  ...
  if (seeded > 0 || repaired > 0) {
    ...
    console.log(
      `[curriculum-toddler-seed] seeded ${seeded} · repaired ${repaired} toddler lesson plan(s)`,
    );
  }

  if (errors.length) {
    console.error("[curriculum-toddler-seed] errors:", errors);
  }

  return {
    seeded,
    repaired,
    skipped: TODDLER_IMPORT_TARGETS.length - missing.length - repaired,
    errors,
  };
}

module.exports = {
  ensureToddlerCurriculumSeeded,
  plansNeedingRepair,
  toddlerPlansMissing,
};
