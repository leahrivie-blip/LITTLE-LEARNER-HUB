...
} = require("../scripts/curriculum-preschool-holiday-import-targets.js");
...
function plansNeedingRepair(curriculum, targets = PRESCHOOL_HOLIDAY_IMPORT_TARGETS) {
  ...
  for (const target of targets) {
    ...
    let source;
    try {
      source = readPreschoolHolidayImportTarget(target);
    } catch {
      continue;
    }
...
async function ensurePreschoolHolidayCurriculumSeeded(deps) {
  const {
    readStore,
    writeStoreAsync,
    writeSiteCurriculum,
  ...
  const missing = preschoolHolidayPlansMissing(curriculum);
  ...
  if (!missing.length && !repairQueue.length) {
    return {
      seeded: 0,
      repaired: 0,
      skipped: PRESCHOOL_HOLIDAY_IMPORT_TARGETS.length,
      errors: [],
    };
  }
  ...
  for (const target of missing) {
    try {
      const parsed = readPreschoolHolidayImportTarget(target);
  ...
  for (const entry of repairQueue) {
    try {
      ...
      if (applyPlan(entry.target, entry.source, existingPlan)) {
        repaired += 1;
        console.log(
          `[curriculum-preschool-holiday-seed] repaired ${entry.target.stableId} (${entry.reason}; ${entry.liveTotal} → ${entry.sourceTotal} activities)`,
        );
      }
    } catch (error) {
  ...
  if (seeded > 0 || repaired > 0) {
    ...
    console.log(
      `[curriculum-preschool-holiday-seed] seeded ${seeded} · repaired ${repaired} preschool holiday lesson plan(s)`,
    );
  }

  if (errors.length) {
    console.error("[curriculum-preschool-holiday-seed] errors:", errors);
  }

  return {
    seeded,
    repaired,
    skipped: PRESCHOOL_HOLIDAY_IMPORT_TARGETS.length - missing.length - repaired,
    errors,
  };
}

module.exports = {
  ensurePreschoolHolidayCurriculumSeeded,
  plansNeedingRepair,
  PRESCHOOL_HOLIDAY_IMPORT_TARGETS,
};
