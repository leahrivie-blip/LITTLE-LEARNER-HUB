...
} = require("../scripts/curriculum-infant-summer-import-targets.js");
...
function plansNeedingRepair(curriculum, targets = INFANT_SUMMER_IMPORT_TARGETS) {
  ...
  for (const target of targets) {
    ...
    let source;
    try {
      source = readInfantSummerImportTarget(target);
    } catch {
      continue;
    }
...
async function ensureInfantSummerCurriculumSeeded(deps) {
  const {
    readStore,
    writeStoreAsync,
    writeSiteCurriculum,
  ...
  const missing = infantSummerPlansMissing(curriculum);
  ...
  if (!missing.length && !repairQueue.length) {
    return {
      seeded: 0,
      repaired: 0,
      skipped: INFANT_SUMMER_IMPORT_TARGETS.length,
      errors: [],
    };
  }
  ...
  for (const entry of repairQueue) {
    try {
      ...
      if (applyPlan(entry.target, entry.source, existingPlan)) {
        repaired += 1;
        console.log(
          `[curriculum-infant-summer-seed] repaired ${entry.target.stableId} (${entry.reason}; ${entry.liveTotal} → ${entry.sourceTotal} activities)`,
        );
      }
    } catch (error) {
  ...
  if (seeded > 0 || repaired > 0) {
    ...
    console.log(
      `[curriculum-infant-summer-seed] seeded ${seeded} · repaired ${repaired} infant summer lesson plan(s)`,
    );
  }

  if (errors.length) {
    console.error("[curriculum-infant-summer-seed] errors:", errors);
  }

  return {
    seeded,
    repaired,
    skipped: INFANT_SUMMER_IMPORT_TARGETS.length - missing.length - repaired,
    errors,
  };
}

module.exports = {
  ensureInfantSummerCurriculumSeeded,
  plansNeedingRepair,
  INFANT_SUMMER_IMPORT_TARGETS,
};
