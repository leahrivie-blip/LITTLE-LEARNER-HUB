/**
 * Startup seed/repair: ensure Toddler holiday Pro published lesson plans exist
...
} = require("../scripts/curriculum-toddler-holiday-import-targets.js");
...
function plansNeedingRepair(curriculum, targets = TODDLER_HOLIDAY_IMPORT_TARGETS) {
  ...
  for (const target of targets) {
    ...
    let source;
    try {
      source = readToddlerHolidayImportTarget(target);
    } catch {
      continue;
    }
...
async function ensureToddlerHolidayCurriculumSeeded(deps) {
  const {
    readStore,
    writeStoreAsync,
    writeSiteCurriculum,
  ...
  const missing = toddlerHolidayPlansMissing(curriculum);
  ...
  if (!missing.length && !repairQueue.length) {
    return {
      seeded: 0,
      repaired: 0,
      skipped: TODDLER_HOLIDAY_IMPORT_TARGETS.length,
      errors: [],
    };
  }
  ...
  for (const entry of repairQueue) {
    try {
      ...
      if (applyPlan(entry.target, entry.source, existingPlan)) {
        repaired += 1;
        console.log(
          `[curriculum-toddler-holiday-seed] repaired ${entry.target.stableId} (${entry.reason}; ${entry.liveTotal} → ${entry.sourceTotal} activities)`,
        );
      }
    } catch (error) {
  ...
  if (seeded > 0 || repaired > 0) {
    ...
    console.log(
      `[curriculum-toddler-holiday-seed] seeded ${seeded} · repaired ${repaired} toddler holiday lesson plan(s)`,
    );
  }

  if (errors.length) {
    console.error("[curriculum-toddler-holiday-seed] errors:", errors);
  }

  return {
    seeded,
    repaired,
    skipped: TODDLER_HOLIDAY_IMPORT_TARGETS.length - missing.length - repaired,
    errors,
  };
}

module.exports = {
  ensureToddlerHolidayCurriculumSeeded,
  plansNeedingRepair,
  TODDLER_HOLIDAY_IMPORT_TARGETS,
};
