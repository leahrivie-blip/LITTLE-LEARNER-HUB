...
} = require("../scripts/curriculum-toddler-core-import-targets.js");
...
function plansNeedingRepair(curriculum, targets = TODDLER_CORE_IMPORT_TARGETS) {
  ...
  for (const target of targets) {
    ...
    let source;
    try {
      source = readToddlerCoreImportTarget(target);
    } catch {
      continue;
    }
...
async function ensureToddlerCoreCurriculumSeeded(deps) {
  const {
    readStore,
    writeStoreAsync,
    writeSiteCurriculum,
  ...
  const missing = toddlerCorePlansMissing(curriculum);
  ...
  if (!missing.length && !repairQueue.length) {
    return {
      seeded: 0,
      repaired: 0,
      skipped: TODDLER_CORE_IMPORT_TARGETS.length,
      errors: [],
    };
  }
  ...
  for (const target of missing) {
    try {
      const parsed = readToddlerCoreImportTarget(target);
  ...
  for (const entry of repairQueue) {
    try {
      ...
      if (applyPlan(entry.target, entry.source, existingPlan)) {
        repaired += 1;
        console.log(
          `[curriculum-toddler-core-seed] repaired ${entry.target.stableId} (${entry.reason}; ${entry.liveTotal} → ${entry.sourceTotal} activities)`,
        );
      }
    } catch (error) {
  ...
  if (seeded > 0 || repaired > 0) {
    ...
    console.log(
      `[curriculum-toddler-core-seed] seeded ${seeded} · repaired ${repaired} toddler core lesson plan(s)`,
    );
  }

  if (errors.length) {
    console.error("[curriculum-toddler-core-seed] errors:", errors);
  }

  return {
    seeded,
    repaired,
    skipped: TODDLER_CORE_IMPORT_TARGETS.length - missing.length - repaired,
    errors,
  };
}

module.exports = {
  ensureToddlerCoreCurriculumSeeded,
  plansNeedingRepair,
  toddlerCorePlansMissing,
};
