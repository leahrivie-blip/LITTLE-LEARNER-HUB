#!/usr/bin/env node
/**
 * Preschool Pro priority imports (Construction Engineers + growing batch).
 * Run: NODE_ENV=test node scripts/test-preschool-priority-imports.js
 */
...
const STORE = path.join(ROOT, "server", `.preschool-priority-test-store-${process.pid}.json`);

const {
  PRESCHOOL_PRIORITY_IMPORT_TARGETS,
  readPreschoolPriorityImportTarget,
} = require("./curriculum-preschool-priority-import-targets.js");
...
function mainSyncChecks() {
  assert.ok(PRESCHOOL_PRIORITY_IMPORT_TARGETS.length >= 1);
  for (const target of PRESCHOOL_PRIORITY_IMPORT_TARGETS) {
...
async function main() {
  ...
  try {
    ...
    assert.match(bootLog, /curriculum-preschool-priority-seed/);
  ...
  console.log("\nAll preschool priority import tests passed.");
}
