#!/usr/bin/env node
/**
 * Import Toddler Family Connections Pro weeks + publish the Curriculum Collection.
 *
 * 1) Builds V3 import files
 * 2) Bulk-imports weeks via the validate-first pipeline
 * 3) Saves/publishes the Toddler age-track series under collectionKey=family-connections
 *
 * Local (default): ephemeral server
 * Production: SITE_URL + ADMIN_EMAIL + ADMIN_PASSWORD + ADMIN_ACCESS_CODE
 *
 * Run:
 *   node scripts/import-toddler-family-connections.js
 *   node scripts/import-toddler-family-connections.js --validate-only
 */
...
const REPORT_PATH = path.join(__dirname, "data/toddler-family-connections-import-report.json");
const COLLECTION_KEY = "family-connections";
const SERIES_ID = "cur-series-toddler-family-connections";
const ID_PREFIX = "cur-lp-toddler-family-connections";

const WEEK_META = [
  { weekNumber: 1, title: "The People Who Love Me", file: "01-toddler-the-people-who-love-me-pro.txt" },
  { weekNumber: 2, title: "My Home & My Family", file: "02-toddler-my-home-and-my-family-pro.txt" },
  { weekNumber: 3, title: "Caring Hearts", file: "03-toddler-caring-hearts-pro.txt" },
  { weekNumber: 4, title: "We Belong Together", file: "04-toddler-we-belong-together-pro.txt" },
];
...
const ADMIN = useRemote
  ...
  : {
      email: "toddler-family-connections@import.local",
      password: "toddler-family-connections-pass",
      code: "toddler-family-connections-code",
    };
const STORE_PATH = path.join(os.tmpdir(), `llh-toddler-family-connections-${crypto.randomBytes(4).toString("hex")}.json`);
...
function startLocalServer() {
  ...
  return spawn(process.execPath, ["server/index.js"], {
    ...
    env: {
      ...
      ADMIN_NAME: "Toddler Family Connections Import",
...
function stablePlanId(title, age = "Toddler") {
...
function buildSeries(planIdsByWeek) {
  return {
    ...
    description:
      "A cohesive four-week Toddler unit that builds from loving relationships to home routines, kindness, and classroom belonging for children ages 1–3.",
    ...
    overallGoals:
      "Strengthen family connections, early communication, cooperative play, and a sense of belonging across four connected toddler weeks.",
    familyConnection:
      "Invite families to share photos, home routines, kindness moments, and friendship stories that connect home and classroom.",
...
async function main() {
  console.log("Building Toddler Family Connections import files…");
  runNode(["scripts/build-toddler-family-connections-imports.js"]);

  const pipelineArgs = [
    "scripts/curriculum-bulk-import-pipeline.js",
    "--dir", "scripts/curriculum-toddler-family-connections-imports",
    "--id-prefix", ID_PREFIX,
    "--report", REPORT_PATH,
  ];
  ...
  try {
    ...
    if (VALIDATE_ONLY) {
      console.log("Toddler Family Connections validation ready. No series written.");
      return;
    }
    ...
    assert(published, "Toddler Family Connections series missing from public library");
    ...
    console.log("Toddler Family Connections collection published:");
