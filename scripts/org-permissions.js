...
function evaluateAccess({
  ...
  const role = memberships.length
    ? normalizeOrgRole(memberships[0].role || actor.role)
    : ORG_ROLES.PARENT_GUARDIAN;
  result.role = role;
  ...
  if (classroomId && !orgWide) {
    ...
    const assigned = activeClassroomIdsForStaff(store, organizationId, actor.userId || memberships[0]?.userId || "");
...
function permissionCatalog() {
  return {
    ...
    rules: {
      ...
      assistants: "Assistants have limited classroom/child actions; no staff, billing, or org-wide management.",
