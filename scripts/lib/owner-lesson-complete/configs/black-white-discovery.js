 ...
 * Lesson ID: cur-lp-infant-black-white-discovery
 * Preserve Free / published. Enrichment draft only.
 * Reuses existing kit + draft printables; upgrades cover to realistic activity photo.
 */
...
const IMAGE_OVERRIDES = {
  "High-Contrast Card Exploration": { imagePlan: "IMAGE_REQUIRED", imageRequirement: "required", imageBriefSetup: "Caregiver holding a high-contrast black-and-white pattern card about 10 inches from an alert infant on a mat — realistic daycare photo, natural light." },
  "Tummy Time Pattern Adventure": { imagePlan: "IMAGE_REQUIRED", imageRequirement: "required", imageBriefSetup: "Infant on tummy looking toward a high-contrast visual strip on a mat with caregiver nearby — realistic infant room." },
  "Mirror & Pattern Discovery": { imagePlan: "IMAGE_REQUIRED", imageRequirement: "required", imageBriefSetup: "Infant looking at unbreakable mirror beside a bold black-white card with caregiver face nearby." },
  "Bold Card Gaze Garden": { imagePlan: "IMAGE_HELPFUL", imageRequirement: "optional", imageBriefSetup: "Infant gazing at one bold contrast card propped safely near a mat." },
  ...
  "Slow Pattern Arc Track": { imagePlan: "IMAGE_HELPFUL", imageRequirement: "optional", imageBriefSetup: "Caregiver slowly arcing a contrast card for infant tracking." },
  "Tummy Contrast Gallery": { imagePlan: "IMAGE_HELPFUL", imageRequirement: "optional", imageBriefSetup: "Tummy-time infant looking at a small gallery of contrast cards." },
  "Black White Cloth Drape": { imagePlan: "IMAGE_HELPFUL", imageRequirement: "optional", imageBriefSetup: "Infant looking at draped black and white cloth squares with caregiver." },
  "Zebra Stripe Soft Book": { imagePlan: "IMAGE_NOT_NEEDED", imageRequirement: "not_needed" },
  "Grasp the Contrast Ring": { imagePlan: "IMAGE_REQUIRED", imageRequirement: "required", imageBriefSetup: "Older infant reaching for a large soft black-white contrast ring toy with caregiver support." },
...
function kitToActivities() {
  ...
  WEEKDAYS.forEach((day) => {
    (activitiesByDay[day] || []).forEach((act) => {
      const img = IMAGE_OVERRIDES[act.title] || {};
      // Map kit cloth title variant
      const liveTitle = act.title === "Black White Cloth Look" ? "Black White Cloth Drape" : act.title;
      out[liveTitle] = {
        ...
        mixedAgeAdaptations: "This week is designed for 0–6 months; older babies may reach/grasp more.",
        ...
        imagePlan: img.imagePlan || (act.imageRequirement === "not_needed" ? "IMAGE_NOT_NEEDED" : "IMAGE_HELPFUL"),
        imageRequirement: img.imageRequirement || act.imageRequirement || "optional",
        imageBriefSetup: img.imageBriefSetup || act.imageBriefSetup || act.description,
        printableDecision: /card|strip|pattern|face|tummy/i.test(act.title) ? "REQUIRED" : "NOT_NEEDED",
      };
      if (act.title === "Black White Cloth Look") {
        out["Black White Cloth Drape"] = out[liveTitle];
      }
    });
  });
  return out;
...
const activities = kitToActivities();
...
const printables = {
  ...
  notes: [
    ...
    },
    {
      ...
      note: "Do not delete; already archived",
    },
  ],
};
