...
const humanActionVisuals = require("./human-action-card-visuals.js");
...
const I = {
  ...
  // NOTE: Primitive circle-head / rectangle-body human icons (carry/share/cheer/gentle/
  // stretch/jump/tiptoe/freeze/fly) were removed. Human-action cards must use
  // human-action-card-visuals.js → Visual Production or hands/object fallback.
  // Object-only cleanup (no geometric people, no explanatory arrows):
  cleanup: `
    <rect x="-80" y="10" width="110" height="70" rx="12" fill="#a16207"/>
    <ellipse cx="-25" cy="10" rx="55" ry="18" fill="#ca8a04"/>
    <circle cx="45" cy="-25" r="28" fill="#ef4444"/>
    <ellipse cx="75" cy="45" rx="24" ry="14" fill="#fda4af"/>`,
...
function iconAt(name, x, y, scale = 1) {
  // Hard block: never render legacy bubble-person SVG keys.
  humanActionVisuals.assertNotPrimitiveHumanSvgIcon(name);
  const body = I[name] || "";
  if (!body) {
    throw new Error(`Unknown printable icon "${name}" (no SVG body).`);
  }
  return `<g transform="translate(${x} ${y}) scale(${scale})">${body}</g>`;
}

/** Card panel art: prefer explicit bodySvg (hands/object/VP placeholder); else iconAt. */
function cardBodyAt(card, x, y, scale = 1) {
  if (card && card.bodySvg) {
    return `<g transform="translate(${x} ${y}) scale(${scale})">${card.bodySvg}</g>`;
  }
  return iconAt(card.icon, x, y, scale);
}
...
function cutSheet2x2(cards, opts = {}) {
  ...
  const cells = cards.slice(0, 4).map((c, i) => {
    ...
    return `
      ...
      ${cardBodyAt(c, x + 260, y + 300, c.scale || 1.35)}
...
function cutSheet2x3(cards, opts = {}) {
  ...
  const cells = cards.slice(0, 6).map((c, i) => {
    ...
    return `
      ...
      ${cardBodyAt(c, x + 260, y + 220, c.scale || 1.1)}
...
async function buildSuperhero(dir) {
  ...
  // Kindness Mission + Hero Movement: NO primitive SVG humans.
  // Pack-level visual family via Visual Production planner; cut-sheet uses
  // hands/object fallback or review placeholder until VP assets are attached.
  const kindnessPlanned = humanActionVisuals.planTeachingCardPackVisuals(
    [
      { title: "Help Carry", subtitle: "Carry a toy for a friend", accent: "#dc2626", panel: "#fef2f2" },
      { title: "Share a Turn", subtitle: "Offer a turn with a toy", accent: "#2563eb", panel: "#eff6ff" },
      { title: "Clean Up", subtitle: "Put one toy away", accent: "#16a34a", panel: "#f0fdf4" },
      { title: "Cheer a Friend", subtitle: "Clap for a friend", accent: "#a855f7", panel: "#faf5ff" },
      { title: "Gentle Hands", subtitle: "Soft hands with friends", accent: "#ea580c", panel: "#fff7ed" },
      { title: "Help Carry", subtitle: "Extra mission card", accent: "#dc2626", panel: "#fef2f2" },
    ],
    { ageBand: "toddler_12_24", activityCategory: "Social-Emotional", setSize: 6 },
  );
  const kindnessCards = kindnessPlanned.cards.map((c) => ({
    title: c.title,
    subtitle: c.subtitle,
    accent: c.accent,
    panel: c.panel,
  ...
  const missions = await svgPng(
    cutSheet2x3(kindnessCards, {
      sheetTitle: `Kindness Mission Cards — cut apart · pack style ${kindnessPlanned.packStyle}`,
    }),
    1200,
    1600,
  );
  await savePdf(
    [async (pdf) => addPortraitPage(pdf, missions, "Kindness Mission Cards", "Circle time + helping missions · rescue play · Visual Production for final art")],
    path.join(dir, "kindness-mission-cards.pdf"),
  );

  const movePlanned = humanActionVisuals.planTeachingCardPackVisuals(
    [
      { title: "Stretch Tall", subtitle: "Reach like a hero", accent: "#2563eb", panel: "#eff6ff" },
      { title: "Jump Soft", subtitle: "Quiet jump", accent: "#16a34a", panel: "#f0fdf4" },
      { title: "Tiptoe", subtitle: "Quiet feet", accent: "#db2777", panel: "#fdf2f8" },
      { title: "Freeze", subtitle: "Statue still", accent: "#ca8a04", panel: "#fefce8" },
      { title: "Fly Arms", subtitle: "Arms out wide", accent: "#7c3aed", panel: "#f5f3ff" },
      { title: "Stretch Tall", subtitle: "Extra move card", accent: "#2563eb", panel: "#eff6ff" },
    ],
    { ageBand: "toddler_12_24", activityCategory: "Movement", setSize: 6, packStyle: kindnessPlanned.packStyle },
  );
  const moveCards = movePlanned.cards.map((c) => ({
    title: c.title,
    subtitle: c.subtitle,
    accent: c.accent,
    panel: c.panel,
  ...
  const moves = await svgPng(
    cutSheet2x3(moveCards, {
      sheetTitle: `Hero Movement Action Cards — cut apart · pack style ${movePlanned.packStyle}`,
    }),
    1200,
    1600,
  );
  await savePdf(
    [async (pdf) => addPortraitPage(pdf, moves, "Hero Movement Action Cards", "Obstacle course and hero movement dance · Visual Production for final art")],
    path.join(dir, "hero-movement-action-cards.pdf"),
  );

  const badge = await svgPng(badgeTemplateSvg(), 1200, 1200);
  await savePdf(
    [async (pdf) => addPortraitPage(pdf, badge, "Super Badge Emblem Template", "Super Badge Creation · Hero Medal — decorate then cut")],
    path.join(dir, "super-badge-emblem-template.pdf"),
  );

  fs.writeFileSync(
    path.join(dir, "human-action-visual-plan.json"),
    JSON.stringify(
      {
        at: new Date().toISOString(),
        note: "No production assets regenerated. Hands/object or review placeholders until VP images are attached.",
        kindness: {
          packStyle: kindnessPlanned.packStyle,
          cards: kindnessPlanned.cards.map((c) => ({
            title: c.title,
            mode: c.mode,
            family: c.visual.family,
            apiVisualStyle: c.visual.apiVisualStyle,
            sceneDescription: c.visual.sceneDescription,
          })),
        },
        movement: {
          packStyle: movePlanned.packStyle,
          cards: movePlanned.cards.map((c) => ({
            title: c.title,
            mode: c.mode,
            family: c.visual.family,
            apiVisualStyle: c.visual.apiVisualStyle,
