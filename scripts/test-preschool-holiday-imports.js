#!/usr/bin/env node
/**
 * Preschool holiday Pro imports (Easter, Fourth of July, New Year's).
 * Run: NODE_ENV=test node scripts/test-preschool-holiday-imports.js
 */
...
const STORE = path.join(ROOT, "server", `.preschool-holiday-test-store-${process.pid}.json`);

const {
  PRESCHOOL_HOLIDAY_IMPORT_TARGETS,
  readPreschoolHolidayImportTarget,
} = require("./curriculum-preschool-holiday-import-targets.js");
...
function mainSyncChecks() {
  assert.equal(PRESCHOOL_HOLIDAY_IMPORT_TARGETS.length, 3);
  for (const target of PRESCHOOL_HOLIDAY_IMPORT_TARGETS) {
...
async function main() {
  ...
  try {
    ...
    assert.match(bootLog, /curriculum-preschool-holiday-seed/);
    ...
    for (const target of PRESCHOOL_HOLIDAY_IMPORT_TARGETS) {
      ...
      assert.ok(linked.some((a) => a.activityCategory === "STEM/Discovery"), `${target.title} missing STEM/Discovery`);
      assert.ok(linked.some((a) => a.activityCategory === "Literacy" || a.activityCategory === "Circle Time"), `${target.title} missing Literacy/Circle Time`);
  ...
  console.log("\nAll preschool holiday import tests passed.");
}
