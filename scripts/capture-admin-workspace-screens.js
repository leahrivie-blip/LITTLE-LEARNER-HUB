#!/usr/bin/env node
/** Capture all seven Admin Workspace screens (desktop). */
  ...
  for (const screen of SCREENS) {
    ...
    if (screen.view === "admin-health") {
      await page.waitForFunction(() => {
        ...
        return /Website|Database|timed out|Retry/i.test(t);
      }, null, { timeout: 15000 });
    }
    if (screen.view === "admin-testers") {
      await page.waitForTimeout(800);
    }
    await page.screenshot({ path: path.join(OUT, `desktop-${screen.file}`), fullPage: true });
