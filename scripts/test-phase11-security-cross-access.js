...
const ARTIFACT_DIR = process.env.PHASE11_ACL_ARTIFACT_DIR
  || (fs.existsSync("/opt/cursor/artifacts")
    ? "/opt/cursor/artifacts/phase11-final-qa/full-audit"
    : path.join(ROOT, "artifacts", "phase11-final-qa", "full-audit"));
...
async function localCrossAccessSuite() {
  ...
  try {
    ...
    // Provider message with explicit foreign/unknown householdId must 404 — no sole-household soft-fallback.
    ...
    assert.equal(crossMsg.status, 404, `explicit unknown householdId must 404, got ${crossMsg.status}: ${crossMsg.text}`);
    const msgsA = await request(port, "GET", "/api/family-hub/messages", { familyToken: tokenA });
    const blobA = JSON.stringify(msgsA.json || {});
    assert.ok(!blobA.includes(probeText), "explicit foreign householdId must not soft-fallback into actor's sole household");
    pass("cross_program_provider_message_denied");

    // Explicit garbage householdId (owned program has households, but id unknown) also 404s
    const unknownIdMsg = await request(port, "POST", "/api/family-hub/provider-messages", {
      email: OWNER_A,
      body: { householdId: "hh-does-not-exist-phase11", body: `UNKNOWN_HH_${crypto.randomBytes(3).toString("hex")}` },
    });
    assert.equal(unknownIdMsg.status, 404, `unknown explicit householdId must 404, got ${unknownIdMsg.status}`);
    pass("provider_message_explicit_unknown_household_404");
