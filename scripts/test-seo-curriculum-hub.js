...
test("curriculum hub keeps cards and access labels without live plan totals", () => {
  ...
  assert.match(page.h1, /Your Next Week of Lesson Plans Is Already Done/);
  assert.match(rendered.bodyHtml, /Your Next Week of Lesson Plans Is Already Done/);
  assert.match(rendered.bodyHtml, /Browse Lesson Plans/);
  assert.match(rendered.bodyHtml, /Start Free/);
  assert.match(rendered.bodyHtml, /Growing Curriculum Library/);
  assert.match(rendered.bodyHtml, /Find a Lesson Plan for Your Classroom/);
  assert.match(rendered.bodyHtml, /#curriculum-browse/);
  ...
  assert.equal((rendered.bodyHtml.match(/<h1[\s>]/g) || []).length, 1);
  assert.doesNotMatch(rendered.bodyHtml, /Daycare Curriculum Built from Real Weekly Lesson Plans/);
  assert.doesNotMatch(rendered.bodyHtml, /Explore ready-to-use weekly daycare curriculum from the current Little Learner Hub library/);
  assert.doesNotMatch(rendered.bodyHtml, /Updated daily with practical plans for infants, toddlers, and preschoolers/);
  assert.doesNotMatch(rendered.bodyHtml, /Library updated/);
  assert.doesNotMatch(rendered.bodyHtml, /Live curriculum snapshot/);
  assert.doesNotMatch(rendered.bodyHtml, /4 lesson plans/);
  assert.doesNotMatch(rendered.bodyHtml, /Current published totals:/);
  assert.doesNotMatch(rendered.bodyHtml, /\d+\s+Infant lesson plans/);
  assert.doesNotMatch(rendered.bodyHtml, /\d+\s+Toddler lesson plans/);
  assert.doesNotMatch(rendered.bodyHtml, /\d+\s+Preschool lesson plans/);
