...
/**
 * Credit-saving Phase 10 visual review:
 * 1) Phone — Family Hub update feed / Daily Report
 * 2) Computer — Provider review/sharing controls
 */
...
const OUT_DIR = process.env.FU_PHASE10_SCREENSHOT_DIR || "/opt/cursor/artifacts/family-updates-phase10";
const ADMIN_EMAIL = "phase10-screens@example.com";
const ADMIN_PASSWORD = "Phase10ScreenPass!99";
const ADMIN_CODE = "phase10-screen-code";
...
async function main() {
  ...
  const storePath = path.join(os.tmpdir(), `llh-fu-phase10-screens-${Date.now()}.json`);
  fs.writeFileSync(storePath, JSON.stringify({
    siteContent: { featureFlags: { directorCenter: true, formsCenter: true, familyHub: true } },
  }, null, 2));

  const port = 8860 + Math.floor(Math.random() * 80);
  ...
  try {
    ...
    await request(port, "POST", "/api/director-center/family-updates/seed", { headers: { Authorization: `Bearer ${token}` }, body: {} });
    ...
    await phonePage.screenshot({ path: path.join(OUT_DIR, "1-family-hub-updates-feed-phone.png"), fullPage: true });
    ...
    const updatesTab = deskPage.locator('[data-dc-tab="family_updates"]');
    if (await updatesTab.count()) {
      await updatesTab.click();
      await deskPage.waitForTimeout(2000);
    }
    await deskPage.screenshot({ path: path.join(OUT_DIR, "2-provider-review-sharing-desktop.png"), fullPage: true });
