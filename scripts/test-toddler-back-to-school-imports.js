#!/usr/bin/env node
/**
 * Toddler Back to School Pro imports + Netflix-style covers.
 * Run: NODE_ENV=test node scripts/test-toddler-back-to-school-imports.js
 */
...
const PORT = 4196 + Math.floor(Math.random() * 20);
const BASE = `http://127.0.0.1:${PORT}`;
const STORE = path.join(ROOT, "server", `.toddler-bts-test-store-${process.pid}.json`);
...
const BTS_TARGETS = TODDLER_PRO_IMPORT_TARGETS.filter((target) => BTS_TITLES.includes(target.title));
...
function mainSyncChecks() {
  assert.equal(BTS_TARGETS.length, 4, "expected 4 Back to School toddler targets");
  for (const target of BTS_TARGETS) {
    ...
    assert.equal(parsed.theme, "Back to School");
    ...
    const resolved = resolveLessonPlanCover({ title: target.title, age: "Toddler", theme: "Back to School" });
...
async function main() {
  ...
  try {
    ...
    for (const target of BTS_TARGETS) {
      ...
      assert.equal(plan.theme, "Back to School");
    ...
    console.log("\nAll Toddler Back to School import tests passed.");
  } finally {
    child.kill("SIGTERM");
