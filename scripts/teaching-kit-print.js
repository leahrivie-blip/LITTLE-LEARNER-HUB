...
(function (root, factory) {
  ...
  function dailyActivitySummaryHtml(summary, index) {
    ...
    return `
      <article class="tk-print-day-activity tk-print-keep">
        ...
        <div class="tk-print-activity-primary">
          <div class="tk-print-day-activity-grid tk-print-activity-grid tk-print-keep-row tk-print-keep">
  ...
  function dailyPlanBody(day, options = {}) {
    ...
    const activityFallback = !summaries
      ...
      : "";
    return `
      <article class="tk-print-day-sheet">
        <header class="tk-print-day-sheet-head tk-print-keep">
  ...
  function activityCardBody(activity, selection, compact) {
    ...
    return `
      <article class="tk-print-activity-card">
        ...
        <div class="tk-print-activity-primary">
          <div class="tk-print-activity-grid tk-print-keep-row tk-print-keep">
  ...
  function assembleEntireBinder(model, selection) {
    ...
    if (sectionIds.has("printables") && partEnabled(selection, "printables")) {
      const items = model.printables || [];
      // When every printable already has an attached PDF that will be merged after the
      // binder pages, omit the generated "Printables / Printable Resources" summary page
      // from the customer Entire Binder. Keep the summary when image printables need a
      // lead-in page or when any printable is still missing its PDF attachment.
      const hasPdfAttachment = (item) => !item?.embedAsImage && Boolean(item?.fileData || item?.fileUrl);
      const needsPrintableSummary = !items.length
        || items.some((item) => item?.embedAsImage || !hasPdfAttachment(item));
      if (needsPrintableSummary) {
        const body = printablesBody(model, selection);
        if (body) chunks.push(page("Printables", "Printables", body, selection.footerLabel));
      }
  ...
  async function buildMergedTeachingKitPdf(kit, options = {}) {
    ...
    const jobApi = binderJobApi();
    const fingerprint = text(built.contentFingerprint);
    if (options.skipArtifactCache !== true && fingerprint && jobApi?.getCachedBinderArtifact) {
      const cached = jobApi.getCachedBinderArtifact(fingerprint);
      if (cached?.bytes?.byteLength) {
        notify({
          stage: "download",
          message: "Preparing download…",
          cached: true,
          pageCount: cached.pageCount || built.pageCount || 0,
        });
        return {
          ok: true,
          reason: "ok_cached",
          ...
          report: {
            ok: true,
            reason: "cached",
            totalPages: cached.pageCount || 0,
            binderEngine: "cache",
            contentFingerprint: fingerprint,
            selectedPrintableIds: built.manifest?.printableIds || [],
            includedPrintableIds: built.manifest?.printableIds || [],
            cached: true,
          },
    ...
    if (fingerprint && jobApi?.cacheBinderArtifact && merged.bytes?.byteLength) {
      jobApi.cacheBinderArtifact(fingerprint, {
        bytes: merged.bytes,
    ...
    return {
      ...
      report: {
        ...
        reflow: binderRendered.reflow || null,
