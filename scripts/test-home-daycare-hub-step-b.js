...
test("shell version bumped for step B", () => {
  assert.match(indexHtml, /SHELL_VERSION = "20260730-ai-guide-audit"/);
  assert.match(indexHtml, /app\.js\?v=20260730-ai-guide-audit/);
});
...
async function main() {
  ...
  try {
    ...
    const appSource = await requestText(port, "/app.js?v=20260730-ai-guide-audit");
