...
const CACHE_V = "20260716-messages-live";
...
test("cache bust versions align for messages-live deploy", () => {
  ...
  assert.match(sw, /llh-shell-v54-messages-live/);
});
...
test("opening an admin conversation marks that member's unread as read", () => {
  ...
  assert.match(handler, /conversationEmail !== userEmail/);
  assert.match(handler, /type !== "message"/);
});

test("member reply creates one admin notification with conversationEmail", () => {
  ...
  assert.match(reply, /fanOutNotificationsAndPush/);
  assert.match(reply, /conversationEmail: identity\.email/);
  // Must not invoke notifyAdminsInApp (comment mentions are fine).
  assert.doesNotMatch(reply, /notifyAdminsInApp\s*\(/);
  assert.match(reply, /const \{ recordTimeline \} = getCommsApi\(\)/);
});
