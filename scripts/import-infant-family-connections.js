#!/usr/bin/env node
/**
 * Import Infant Family Connections Pro weeks + publish the Curriculum Collection.
 *
 * 1) Builds V3 import files
 * 2) Bulk-imports weeks via the validate-first pipeline
 * 3) Saves/publishes the Infant age-track series under collectionKey=family-connections
 *
 * Local (default): ephemeral server
 * Production: SITE_URL + ADMIN_EMAIL + ADMIN_PASSWORD + ADMIN_ACCESS_CODE
 *
 * Run:
 *   node scripts/import-infant-family-connections.js
 *   node scripts/import-infant-family-connections.js --validate-only
 */
...
const REPORT_PATH = path.join(__dirname, "data/family-connections-import-report.json");
const COLLECTION_KEY = "family-connections";
const SERIES_ID = "cur-series-infant-family-connections";
const ID_PREFIX = "cur-lp-infant-family-connections";

const WEEK_META = [
  { weekNumber: 1, title: "The People Who Love Me", file: "01-infant-the-people-who-love-me-pro.txt" },
  { weekNumber: 2, title: "My Home & My Family", file: "02-infant-my-home-and-my-family-pro.txt" },
  { weekNumber: 3, title: "Caring Hearts", file: "03-infant-caring-hearts-pro.txt" },
  { weekNumber: 4, title: "We Belong Together", file: "04-infant-we-belong-together-pro.txt" },
];
...
const ADMIN = useRemote
  ...
  : {
      email: "family-connections@import.local",
      password: "family-connections-pass",
      code: "family-connections-code",
    };
const STORE_PATH = path.join(os.tmpdir(), `llh-family-connections-${crypto.randomBytes(4).toString("hex")}.json`);
...
function startLocalServer() {
  ...
  return spawn(process.execPath, ["server/index.js"], {
    ...
    env: {
      ...
      ADMIN_NAME: "Family Connections Import",
...
function stablePlanId(title, age = "Infant 0-12 Months") {
...
function buildSeries(planIdsByWeek) {
  return {
    ...
    description:
      "A cohesive four-week Infant unit that builds from loving relationships to home routines, kindness, and classroom belonging.",
    ...
    overallGoals:
      "Strengthen secure attachments, early communication, sensory exploration, and a sense of belonging across four connected weeks.",
    familyConnection:
      "Invite families to share photos, bedtime routines, and loving messages that connect home and classroom.",
    weeks: WEEK_META.map((week) => ({
...
async function main() {
  console.log("Building Family Connections import files…");
  runNode(["scripts/build-infant-family-connections-imports.js"]);

  const pipelineArgs = [
    "scripts/curriculum-bulk-import-pipeline.js",
    "--dir", "scripts/curriculum-infant-family-connections-imports",
    "--id-prefix", ID_PREFIX,
    "--report", REPORT_PATH,
  ];
  ...
  // For local import, let the pipeline own its ephemeral server, then open another
  // local server only if we need to attach the series against the same store.
  // Simpler path: when not remote, run pipeline validate/import first; for series,
  // re-import against a shared server we control.
  let child = null;
  try {
    ...
    if (VALIDATE_ONLY) {
      console.log("Family Connections validation ready. No series written.");
      return;
    }
    ...
    const published = publicSeries.find((entry) => entry.id === SERIES_ID);
    assert(published, "Family Connections series missing from public library");
    assert(published.collectionKey === COLLECTION_KEY, "collectionKey missing on public series");
    assert((published.weeks || []).filter((week) => week.lessonPlanId).length === 4, "expected 4 linked weeks");

    console.log("Family Connections collection published:");
