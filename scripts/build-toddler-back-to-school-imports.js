#!/usr/bin/env node
/**
 * Normalize Toddler Back to School raw pastes into v3 curriculum import files.
 *
 * Reads: scripts/curriculum-import-samples/toddler-back-to-school-raw.txt
 * Writes: scripts/curriculum-toddler-pro-imports/08–11-toddler-*-pro.txt
 *
 * Run: node scripts/build-toddler-back-to-school-imports.js
 */
const fs = require("fs");
...
const RAW_PATH = path.join(__dirname, "curriculum-import-samples/toddler-back-to-school-raw.txt");
...
const CATEGORY_MAP = {
  ...
  "social-emotional celebration": "Open-Ended Exploration",
  "classroom exploration": "Open-Ended Exploration",
  "life skills": "Open-Ended Exploration",
  "practical life": "Open-Ended Exploration",
  "blocks & construction": "STEM/Discovery",
  "blocks and construction": "STEM/Discovery",
  "mindfulness": "Sensory Play",
  "mindfulness & movement": "Music & Movement",
  "mindfulness and movement": "Music & Movement",
  "obstacle course": "Gross Motor",
  "sorting & cognitive development": "STEM/Discovery",
  "sorting and cognitive development": "STEM/Discovery",
...
function normalizePlanText(raw) {
  ...
  for (const line of lines) {
    ...
    // Multi-line expand from inline "LABEL:\nvalue"
  ...
  // Ensure learning objectives / domains / vocabulary use plain lines
  let text = out.join("\n");
  text = text.replace(/\n{3,}/g, "\n\n").trim() + "\n";
  // Force PLAN/STATUS/AGE/THEME if somehow missed
  ...
  // Alias matches parser day-field names
  text = text.replace(/^DAILY_OBSERVATION_OPPORTUNITIES:$/gm, "DAILY_OBSERVATIONS:");
  // Move trailing day fields (after last activity) into the day preamble so the parser keeps them
  text = relocateTrailingDayFields(text);
  // Ensure blank line before weekday headers
...
function main() {
  ...
  chunks.forEach((chunk, index) => {
    ...
    report.push({ file: meta.file, title: parsed.data.title, activityCount, plan: parsed.data.plan, status: parsed.data.status });
    console.log(`Wrote ${meta.file} (${parsed.data.title}, ${activityCount} activities)`);
  });

  const reportPath = path.join(__dirname, "data/toddler-back-to-school-import-report.json");
  fs.mkdirSync(path.dirname(reportPath), { recursive: true });
  fs.writeFileSync(reportPath, JSON.stringify({ generatedAt: new Date().toISOString(), plans: report }, null, 2));
  console.log(`\nSuccess: ${report.length} Toddler Back to School import files ready.`);
}
