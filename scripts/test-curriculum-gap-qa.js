 ...
 * Run: node scripts/test-curriculum-gap-qa.js
 */
const fs = require("fs");
...
async function runBrowserGaps(baseUrl, lessonTitle, proTitle) {
  ...
  // Logged-out blocked from lessons
  {
    ...
    assert(modalOpen, "Logged-out user should see login modal for lessons");
    await page.close();
  }
  ...
  // Pro user unlocks Pro lesson
  {
    ...
    assert(!/preview/i.test(viewText), "Pro user should see View not Preview");
  ...
  // Free user: no View Activities on locked Pro card
  {
  ...
  // Back buttons on lesson library and viewer
  {
    const page = await personaPage("pro");
    await page.evaluate(() => { if (typeof setView === "function") setView("lessons"); });
    await page.waitForSelector('#view-lessons .back-button[data-view="home"]', { timeout: 15000 });
    await page.fill("#lessonPlanSearch", proTitle);
...
async function main() {
  ...
  assert(!guestViews.includes('"lessons"'), "lessons should not be guest-allowed");
