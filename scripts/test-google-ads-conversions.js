...
assert.equal(calls[1][2].currency, "USD");
assert.equal(ads.emit("paid_subscription", { dedupeKey: "paid-1" }), false);
assert.equal(ads.emit("trial_start", { dedupeKey: "trial-1" }), false);
...
assert.ok(!server.includes("googleAdsConversionOutbox"));
assert.ok(!app.includes("paid_subscription"));
