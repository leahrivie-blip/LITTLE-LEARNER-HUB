...
The server tracks Founding Member inventory in `foundingMembers[]` (local JSON or Postgres). Cap is `FOUNDING_MEMBER_LIMIT` (production closeout: **46**, leaving 2 new spots after 44 claimed). Checkout uses an atomic server-side claim so two people cannot take the final spot. When remaining hits 0, Founding closes everywhere and new Pro is `$19.99/month`. Existing Founding Members stay grandfathered at `$9.99/month` for life.
